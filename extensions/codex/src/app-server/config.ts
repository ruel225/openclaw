...
export function hasCodexRemoteExecutionStartOptions(
  options: Pick<CodexAppServerStartOptions, "env">,
): boolean {
  return Boolean(options.env?.[CODEX_EXEC_SERVER_NOISE_REGISTRY_URL_ENV_VAR]);
}
...
function resolveCodexAppServerRemoteExecution(params: {
  ...
  const fingerprint = createHash("sha256")
    .update("openclaw:codex:remote-execution:v1")
    .update("\0")
    .update(registryUrl)
    .update("\0")
