...
import {
  ...
  threadStartResult,
} from "./bounded-turn.test-helpers.js";
...
describe("runBoundedCodexAppServerTurn settled finalization isolation", () => {
  ...
  it("keeps its configured budget across a forward wall-clock correction during startup", async () => {
    const fake = createClientFactory({ completeTurn: false });
    const baseMs = Date.now();
    let correctionMs = 0;
    const dateNow = vi.spyOn(Date, "now").mockImplementation(() => baseMs + correctionMs);
    ...
    try {
      ...
      // The correction happened during startup, so the full bound must still
      // elapse before the turn reports its own timeout.
  ...
  it("keeps a pending turn bounded across a backward wall-clock correction during startup", async () => {
    const fake = createClientFactory({ completeTurn: false });
    const baseMs = Date.now();
    let correctionMs = 0;
    const dateNow = vi.spyOn(Date, "now").mockImplementation(() => baseMs + correctionMs);
    ...
    let budgetGuard: NodeJS.Timeout | undefined;
    try {
      const run = runBoundedCodexAppServerTurn({
        model: { mode: "required", id: "gpt-5.4" },
        timeoutMs: 1_000,
      ...
      await expect(
        Promise.race([
          ...
          new Promise<never>((_, reject) => {
            budgetGuard = setTimeout(
              () => reject(new Error("turn outlived its configured bound")),
              4_000,
            );
          }),
        ]),
