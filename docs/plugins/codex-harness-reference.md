...
| Field                                           | Default                                                | Meaning                                                                                                                                                                                                                                                                                                                                                          |
| ----------------------------------------------- | ------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `transport`                                     | `"stdio"`                                              | `"stdio"` spawns the Gateway-owned Codex app-server. `"websocket"` connects to `url`, but upstream marks TCP WebSocket transport experimental and unsupported; it is not a production topology.                                                                                                                                                                  |
...
| `remoteWorkspaceRoot`                           | unset                                                  | Remote Codex execution workspace root. OpenClaw preserves the current cwd suffix under this root and fails closed when the local cwd cannot be projected safely.                                                                                                                                                                                                 |
...
| `experimental.remoteExecution`                  | unset                                                  | Preview authenticated registry-backed remote execution. Requires stdio, `remoteWorkspaceRoot`, and Codex app-server 0.142.0 or newer.                                                                                                                                                                                                                            |
| `experimental.remoteExecution.registryUrl`      | required when enabled                                  | HTTPS environment-registry base URL. Plain HTTP is accepted only for loopback; credentials, queries, and fragments are rejected.                                                                                                                                                                                                                                 |
| `experimental.remoteExecution.environmentId`    | required when enabled                                  | Registry environment id exposed by `codex exec-server`.                                                                                                                                                                                                                                                                                                          |
| `experimental.remoteExecution.authToken`        | required when enabled                                  | App-server registry bearer token; accepts a literal or SecretInput.                                                                                                                                                                                                                                                                                              |
| `experimental.remoteExecution.chatgptAccountId` | unset                                                  | Optional registry account id; accepts a literal or SecretInput.                                                                                                                                                                                                                                                                                                  |
...
Remote execution has a separate stable-version floor of `0.142.0`.
...
`remoteWorkspaceRoot` is set, the registry uses HTTPS or true loopback HTTP,
and all SecretInputs resolve. It also rejects remote-execution environment
names in `clearEnv`, `networkProxy`, `sandboxExecServer`, Computer Use, an
active OpenClaw sandbox, and local stdio MCP servers. Bounded private turns
clear remote routing and remain local. Use an intentionally reachable HTTP MCP
endpoint when this mode needs MCP. Codex requirements remain owned by the
Gateway-side app-server. Initialization does not prove executor registration;
deployment readiness must check registry/executor health separately. Keep
registry credentials out of `shell_environment_policy.set` and launch the
remote exec-server with a sanitized ambient environment so credentials are not
inherited by its child commands.
