...
<Steps>
  ...
  <Step title="Run the guided setup">
    ...
    ```
    Choose **Set up a Signal Docker container** for the recommended path, **Use local signal-cli** for a native daemon, or **Connect to an existing Signal server** when you already manage the server.
    The Docker path creates or starts a loopback-only `bbernhard/signal-cli-rest-api` container, shows a linked-device QR/link when needed, waits for the linked account, and saves the selected account with `apiMode: "container"`.
  </Step>
  <Step title="Link or register the account">
    - **Docker wizard:** follow the QR/link shown by `openclaw channels add`; no separate `signal-cli link` command is needed.
