...
1. Install `imsg` on the Mac that runs Messages.app:
   ...
   OpenClaw does not install or upgrade `imsg`; for the usual local setup, repeat the Homebrew update on the signed-in Messages Mac. If you use the less common SSH-wrapper topology, run the update in that same remote user context. If `imsg chats` fails with `unable to open database file`, empty output, or `authorization denied`, grant Full Disk Access to the terminal, editor, Node process, Gateway service, or SSH parent process that launches `imsg`, then reopen that parent process.
